...
<!-- cSpell: words gotmpl appstage -->

karmafun is a
[kustomize plugin](https://kubectl.docs.kubernetes.io/guides/extending_kustomize/)
as well as a kustomize wrapper providing a set of [KRM Functions] that you can
use to perform in place transformation in your kustomize projects.
...
- **Values and secrets injection** — a `values.yaml` file and an optional
  SOPS‑encrypted `secrets.sops.yaml` file are loaded, merged, and made available
  inside kustomization files as Go template variables.
...
When no `--output-directory` is given, the built YAML is printed to stdout, just
like `kustomize build`.

...
var _ framework.ResourceListProcessor = (*Processor)(nil)

// PluginBuilder is responsible for building plugins based on their Gvk.
// It is used by the Processor to obtain plugin instances.
type PluginBuilder interface {
	Build(resid.Gvk) (resmap.Configurable, error)
}

type builderFunc func(resid.Gvk) (resmap.Configurable, error)

func (f builderFunc) Build(id resid.Gvk) (resmap.Configurable, error) {
	return f(id)
}
...
func NewProcessor(builder PluginBuilder) *Processor {
	return &Processor{
		builder: builder,
	}
}
...
func NewDefaultProcessor() *Processor {
	return &Processor{
		builder: builderFunc(plugins.MakeBuiltinPlugin),
	}
}
...
func (p *Processor) Process(rl *framework.ResourceList) error {
	...
	plugin, err := p.builder.Build(resid.GvkFromNode(config))
